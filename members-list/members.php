...
class tern_members {
	...
	function members($a,$e=true) {
		...
		//  DESIGN STUDIOS ->  remove filter for users //
	...
	function alpha($e=false) {
		...
		$r = '<div class="tern_members_alpha">Search alphabetically <span>(by first name)</span>:<br /><ul>';
		foreach($a as $v) {
			unset($c);
			$c = '';
			...
			$r .= '<li><a '.$c.' href="'.$this->url.'&page=1&query='.$v.'&type=alpha&sort=first_name">'.strtoupper($v).'</a></li>';
		}
		$r .= '</ul></div>';
		if($e) { echo $r; }
		return $r;
	...
	function viewing($a,$e=false) {
		...
		$m = '.';
		if(isset($this->type) and $this->type == 'alpha') {
			$m = ' whose first names begin with the letter "'.strtoupper($q).'".';
		}
...
------------------------------------------------------------------------------------------------*/
	...
	private function list_loop_add_gravatar($b=false) {
		$biddyname=get_author_posts_url($this->member['ID']);
		$biddyname = str_replace('author', 'forums/users', $biddyname);
		if((isset($this->o['gravatars']) and $this->o['gravatars']) or $b) {
			$this->ml .= '<div class="WP-ml-field-img">';
			$this->ml .= '<a href="'.$biddyname.'">';
	...
	private function list_loop_field_value() {
		if($this->field == 'first_name' and $this->field_next == 'last_name') {
			$this->ml .= !empty($this->value) ?'<div class="WP-ml-field-value name-tweak">'.$this->value.'</div>' : '';
		}
		elseif($this->field == 'last_name' and $this->field_last == 'first_name') {
			$this->ml .= !empty($this->value) ?'<div class="WP-ml-field-value name-tweak">'.$this->value.'</div>' : '';
		}
	...
	private function list_loop_field_value_set() {
		...
		if($this->field == 'description' and !empty($this->value)) {
			...
			$this->value .= '<a class="WP-ml-button">&rarr;</a>';
		}
	}
