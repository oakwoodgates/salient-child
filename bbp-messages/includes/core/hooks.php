...
class BBP_messages_hooks
{
	...
	function __construct() {
		...
		add_filter('bbpm_the_message', function( $content ) {
		...
		add_filter('bbp_template_after_user_profile', function() {
			...
			if( ! get_userdata( $user_id ) || $user_id == wp_get_current_user()->ID )
				return;
			...
			?>
				...
				<p>
					<a href="<?php echo bbpm_messages_base( get_userdata( $user_id )->user_nicename . '/', wp_get_current_user()->ID ); ?>">Send <br><span style="font-size: 28px;font-weight: 600;color: #fff;font-family: Montserrat;"> <?php echo get_userdata( $user_id )->first_name; ?> </span><br>a message</a>
				</p>
		...
		add_action('bbp_theme_after_reply_author_details', function() {
			...
			?>
				...
				<p>
					<a href="<?php echo bbpm_messages_base( get_userdata( $user_id )->user_nicename . '/', wp_get_current_user()->ID ); ?>">Send <?php echo get_userdata( $user_id )->first_name; ?> a message</a>
				</p>
